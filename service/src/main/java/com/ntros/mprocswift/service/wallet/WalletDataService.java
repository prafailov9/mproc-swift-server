...
public class WalletDataService implements WalletService {
  ...
  protected static final Logger log = LoggerFactory.getLogger(WalletDataService.class);
    ...
    return CompletableFuture.supplyAsync(
        () ->
            walletRepository
                .findById(walletId)
                .orElseThrow(
                    () -> new WalletNotFoundException("Wallet not found for id: " + walletId)),
        executor);
  }

  @Override
  public CompletableFuture<Wallet> getWalletByCurrencyCodeAndAccountNumber(
                ...
                .orElseThrow(
                    () ->
                        new WalletNotFoundException(
                            String.format(
                                "Wallet not found for [%s, %s]", currencyCode, accountNumber))),
        executor);
  }

  @Override
  public CompletableFuture<Wallet> getWalletByCurrencyNameAndAccountId(
      String currencyName, int accountId) {
    return CompletableFuture.supplyAsync(
        () ->
            walletRepository
                .findByCurrencyNameAccountId(currencyName, accountId)
                .orElseThrow(
                    () ->
                        new WalletNotFoundForCurrencyAndAccountException(currencyName, accountId)),
        executor);
  }

  @Override
  public CompletableFuture<Wallet> getWalletByCurrencyCodeAndAccountId(
      String currencyCode, int accountId) {
    return CompletableFuture.supplyAsync(
        () ->
            walletRepository
                .findByCurrencyCodeAccountId(currencyCode, accountId)
                .orElseThrow(
                    () ->
                        new WalletNotFoundForCurrencyAndAccountException(currencyCode, accountId)),
        executor);
  }
  ...
  public CompletableFuture<List<Wallet>> getAllWalletsByAccount(final int accountId) {
    return CompletableFuture.supplyAsync(
            () -> walletRepository.findAllByAccount(accountId), executor)
        .exceptionally(
            ex -> {
  ...
  public CompletableFuture<Wallet> createWallet(WalletDTO walletDTO) {
    return CompletableFuture.supplyAsync(() -> walletConverter.toModel(walletDTO), executor)
        .thenApply(
            wallet ->
                getAndSetCurrencyAccount(
                    walletDTO.getCurrencyCode(), walletDTO.getAccountNumber(), wallet))
  ...
  public CompletableFuture<Integer> deleteWallet(UniqueWalletDTO uniqueWalletDTO) {
    return CompletableFuture.supplyAsync(
        () -> delete(uniqueWalletDTO.getCurrencyCode(), uniqueWalletDTO.getAccountNumber()),
        executor);
  }
    ...
    return CompletableFuture.runAsync(
        () -> {
          try {
            ...
            log.error(error, ex);
            // rethrowing as a completion exception so it is caught by exceptionally block in
            // thenCombine
    ...
    try {
      ...
      if (affectedRows > 1) {
        ...
        throw new DataConstraintViolationException(
            String.format("Deleted multiple wallets for [%s, %s]", code, an));
      }
  ...
  private Wallet getAndSetCurrencyAccount(
      String currencyCode, String accountNumber, Wallet wallet) {
    Currency currency =
        currencyRepository
            ...
            .orElseThrow(
                () ->
                    new CurrencyNotFoundException(
                        String.format("Currency [%s] doesnt exist.", currencyCode)));
    Account account =
        accountRepository
            ...
            .orElseThrow(
                () ->
                    new AccountNotFoundException(
                        String.format("Account with AN:[%s] doesnt exist.", accountNumber)));
  ...
  private Wallet updateActiveWallet(Wallet wallet) {
    if (wallet.isMain()) {
      wallet
          .getAccount()
          .setWallets(
              wallet.getAccount().getWallets().stream()
  ...
  private Wallet create(Wallet wallet) {
    try {
      ...
      throw new WalletCreateFailedException(
          String.format(
              "Could not create wallet for currency and account [%s, %s]",
              wallet.getCurrency().getCurrencyCode(),
              wallet.getAccount().getAccountDetails().getAccountNumber()),
          ex.getCause());
    }
  }
}
