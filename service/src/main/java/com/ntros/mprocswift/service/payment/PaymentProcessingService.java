...
public class PaymentProcessingService implements PaymentService {
    ...
    public PaymentProcessingService(MerchantService merchantService,
                                    ...
                                    TransactionService transactionService, WalletService walletService, AccountService accountService) {
    ...
    public AuthorizePaymentResponse authorizePayment(AuthorizePaymentRequest paymentRequest) {
        ...
        try {
            ...
            Wallet lockedWallet = walletService.getLockedWallet(getAvailableWallet(card, paymentRequest).getWalletId());

            // run FX conversion if needed
            BigDecimal requestedAmount = getAmount(lockedWallet, paymentRequest);
            ...
            BigDecimal totalReserved = existingHolds.add(requestedAmount);

            if (!lockedWallet.hasAvailableBalance(totalReserved)) {
                throw new InsufficientFundsException(
                        format("Insufficient funds. Requested (incl. holds): %s, Balance: %s",
                                totalReserved, lockedWallet.getBalance())
                );
            }
            AuthPaymentContext ctx = new AuthPaymentContext(card, merchant, lockedWallet, requestedAmount, currency);
            ...
            log.error("Authorization hold request failed. cardIdHash={}, merchant={}, amount={}, currency={}",
                    paymentRequest.getCardIdentifier(), paymentRequest.getMerchant(), paymentRequest.getAmount(), paymentRequest.getCurrency(), ex);
    ...
    public HoldSettlementResponse settleHold(HoldSettlementRequest holdSettlementRequest) {
        // idempotency check
        HoldSettlement existingSettlement = transactionService.getHoldSettlement(holdSettlementRequest.getAuthCode());
        AuthorizedHold hold = transactionService.getAuthorizedHold(holdSettlementRequest.getAuthCode());
        if (existingSettlement != null) {
            log.info("Hold already settled for this AuthCode: {}; HoldSettlement: {}", holdSettlementRequest.getAuthCode(), existingSettlement);
        ...
        if (!baseTx.getStatus().getStatusName().equals(AuthCodeType.AUTHORIZED.name())) {
            throw new IllegalArgumentException(String.format("Settlement not authorized. Tx: %s", baseTx));
        }
        ...
        if (!wallet.hasAvailableBalance(amountToSettle)) {
            throw new InsufficientFundsException(String.format("Wallet {%s} has insufficient funds. Amount to settle: %s, wallet balance: %s", wallet, amountToSettle, wallet.getBalance()));
        }
        ...
        transactionService.settleHold(cardAuth, hold,
                new AuthPaymentContext(cardAuth.getCard(), cardAuth.getMerchant(), wallet, amountToSettle, wallet.getCurrency()));
    ...
    private static HoldSettlementResponse getHoldSettlementResponse(AuthorizedHold hold) {
        ...
        response.setSettledAmount(baseTx.getAmount().toString());
    ...
    /**
     * try to get wallet from request(req.currency)
     * if not exist, default to main wallet, or first available wallet. Run fx conversion if needed.
     */
    private Wallet getAvailableWallet(Card card, AuthorizePaymentRequest request) {
        ...
        if (account.getWallets().isEmpty()) {
            throw new IllegalArgumentException(String.format("No available wallets for Account: %s", account));
        }
        return account.getWalletByCurrencyCode(request.getCurrency())
                .orElse(account.getMainWallet()
                        .orElse(account.getWallets().get(0)));
    }
