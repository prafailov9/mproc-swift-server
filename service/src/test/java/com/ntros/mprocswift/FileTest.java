...
public class FileTest {
  ...
  public void setOneMainWalletForEachAccountTest() throws IOException {
    ...
    Path rootPath = Paths.get(TEST_FILES_DIR);
  ...
  /**
   * should have unique 8-12 digit account_numbers and 9 digit routing numbers insert into
   * account_details (account_name, account_number, routing_number, iban, bicswift, bank_address)
   * values ('Bank of America Corporation', 14, 9, 'CR27 4617 0625 5742 6037 6', '2W30X3Z', '507
   * Barby Park');
   */
  @Test
  public void fixAccountDataTest() throws IOException {
    ...
    accountNumbers.add(
        String.valueOf(rng.nextLong(ACCOUNT_BOUND - ACCOUNT_ORIGIN) + ACCOUNT_ORIGIN));
    routingNumbers.add(
        String.valueOf(rng.nextLong(ROUTING_BOUND - ROUTING_ORIGIN) + ROUTING_ORIGIN));
        ...
        .forEach(
            line -> {
              ...
              List<String> numberValues =
                  Stream.of(valuesString.split(", "))
                      .filter(
                          value ->
                              value.matches("[0-9]+")) // checks if the line contains only numbers
                      .toList();
              ...
              // using last digit of the account number as starting point since the routing number
              // is
              // located right after account number in the values string
              valuesString =
                  valuesString
                      .replace("', " + numberValues.get(0), "', '" + account + "'")
                      .replace(
  ...
  /**
   * insert into address (address_id, country, city, street_name, street_number, postal_code) values
   * (1, 'Benin', 'Tchaourou', 'Waxwing', '33', null);
   */
  @Test
        ...
        .forEach(
            str -> {
              ...
              for (int i = 1; i < values.length; i++) {
                ...
                if (!values[i].equals(
                    "null")) { // some values for postal_code are "null" as strings.
  ...
  /**
   * address hash for efficiently identifying a unique address. 0xff & b: 0xff (which is 11111111 in
   * binary or 255 in decimal). A byte is 8 bits and has a range from -128 to 127. This means that
   * when you deal with byte values, they are signed, and you might get negative values. The & 0xff
   * operation is used to convert the signed byte into an unsigned value. The 0xff effectively masks
   * the byte so that only the lower 8 bits (one byte) are considered, and any sign bit is ignored.
   * This ensures that the byte is treated as a value between 0 and 255 instead of -128 to 127.
   */
